"use client"

import type { Doctor } from "@/lib/supabase/types"
import { Card, CardContent, CardDescription, CardHeader, CardTitle } from "@/components/ui/card"
import { ShiftCard } from "@/components/dashboard/shift-card"
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs"
import { useT } from "@/lib/i18n/language-provider"

interface ShiftsListProps {
  shifts: any[]
  currentDoctor: Doctor
}

export function ShiftsList({ shifts, currentDoctor }: ShiftsListProps) {
  const t = useT()
  const newShifts = shifts.filter((s) => s.status === "new")
  const freeShifts = shifts.filter((s) => s.status === "free" || s.status === "free_pending")
  const confirmed = shifts.filter((s) => s.status === "confirmed")

  return (
    <Card>
      <CardHeader>
        <CardTitle>{t("shiftsList.title")}</CardTitle>
        <CardDescription>{t("shiftsList.description")}</CardDescription>
      </CardHeader>
      <CardContent>
        <Tabs defaultValue="new" className="w-full">
          <TabsList className="grid w-full grid-cols-3">
            <TabsTrigger value="new">{t("shiftsList.tabNew")} ({newShifts.length})</TabsTrigger>
            <TabsTrigger value="free">{t("shiftsList.tabFree")} ({freeShifts.length})</TabsTrigger>
            <TabsTrigger value="confirmed">{t("shiftsList.tabConfirmed")} ({confirmed.length})</TabsTrigger>
          </TabsList>

          <TabsContent value="new" className="space-y-4 mt-4">
            {newShifts.length === 0 ? (
              <p className="text-center text-slate-500 py-8">{t("shiftsList.emptyNew")}</p>
            ) : (
              newShifts.map((shift) => <ShiftCard key={shift.id} shift={shift} doctorId={currentDoctor.id} />)
            )}
          </TabsContent>

          <TabsContent value="free" className="space-y-4 mt-4">
            {freeShifts.length === 0 ? (
              <p className="text-center text-slate-500 py-8">{t("shiftsList.emptyFree")}</p>
            ) : (
              freeShifts.map((shift) => <ShiftCard key={shift.id} shift={shift} doctorId={currentDoctor.id} />)
            )}
          </TabsContent>

          <TabsContent value="confirmed" className="space-y-4 mt-4">
            {confirmed.length === 0 ? (
              <p className="text-center text-slate-500 py-8">{t("shiftsList.emptyConfirmed")}</p>
            ) : (
              confirmed.map((shift) => <ShiftCard key={shift.id} shift={shift} doctorId={currentDoctor.id} />)
            )}
          </TabsContent>
        </Tabs>
      </CardContent>
    </Card>
  )
}
